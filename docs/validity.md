...
| `camera_timestamp_non_monotonic` | For one exact camera identity across the recording, current minus previous effective camera timestamp is zero or negative. Previous/current/delta are reported for selected and unselected batches. The raw frame records whether the timestamp came from `camera_timestamp` or the explicitly enabled batch fallback; grid time is never substituted. |
